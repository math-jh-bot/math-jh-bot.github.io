...
만일 $\mathfrak{a}$에서 $\mathfrak{b}$의 부분집합으로의 bijection을 $i$라 하면 $i(\mathfrak{a})\subset\mathfrak{b}$이고 $\mathfrak{a}$와 $i(\mathfrak{a})$은 equipotent하다. 따라서 $i(\mathfrak{a})$와 $\mathfrak{b}$ 사이의 bijection이 존재함을 보이면 충분하다.  
$\mathfrak{b}\leq\mathfrak{a}$이므로, $\mathfrak{b}$에서 $\mathfrak{a}$의 부분집합으로의 bijection이 존재하고, 이는 $\mathfrak{b}$에서 $\mathfrak{a}$로의 injection으로 볼 수 있다. 힌퍈. $\mathfrak{a}$는 $i(\mathfrak{a})$와 equipotent하므로, 이 둘 사이의 bijection을 앞선 injection과 합성하면 $\mathfrak{b}$에서 $i(\mathfrak{a})$로의 injection을 얻는다. 이를 $f$라 하자. 이제 $C_0=\mathfrak{b}\setminus i(\mathfrak{a})$라 하고, 귀납적으로 $C\_{n+1}=f(C_n)$으로 정의하고 $C=\bigcup C_n$이라 하자. 우리는 $h:\mathfrak{b}\rightarrow i(\mathfrak{a})$를 다음의 식 

$$h(x)=\begin{cases} f(x)&x\in C\\ x&x\not\in C\end{cases}$$

으로 정의하고, $\mathfrak{b}$에서 $i(\mathfrak{a})$로의 bijection임을 보일 것이다.
...
또, $h$는 injective이다. 만일 $h(x)=h(y)$라 한다면, $x,y\in C$인 경우는 $f(x)=f(y)$가 되고, 그럼 $f$가 injection이므로 $x=y$이다. 그리고 $x,y\not\in C$인 경우는 자명하게 $x=y$가 된다.  
자명하지 않은 경우는 하나가 $C$의 원소이고 다른 하나는 $C$의 원소가 아닐 때이다. $x\in C$라 하고 $y\not\in C$라 하자. 그럼 어떤 $n$에 대하여 $x\in C_n$이고, 특히 $h(x)=f(x)\in C\_{n+1}\subset C$이므로 $h(x)\in C$이다. 한편 $h(y)=y$인데, 가정에 의해 $y\not\in C$이므로 $h(x)=h(y)$라는 가정에 모순이다. 따라서 $x=y$가 항상 성립하고 $h$는 injection이다.

마지막으로 $h$가 surjection임을 보이자. 임의의 $y\in i(\mathfrak{a})$에 대하여, $y\in C$이거나 $y\not\in C$이다. 만일 $y\not\in C$라면 $h$의 정의에 의해 $h(y)=y$이다. 만일 $y\in C$라면, 어떤 $n\geq 1$에 대하여 $y\in C\_{n}$이다. ($y\in C_0=\mathfrak{b}\setminus i(\mathfrak{a})$는 불가능하므로) 따라서 $y\in f(C\_{n-1})$이 되어 $y=f(x)$인 $x\in C\_{n-1}$이 존재한다. 이 $x$는 $C$의 원소이기도 하므로, $h(x)=f(x)=y$이고, 따라서 $h$는 surjection이다.
...
[§집합의 합, 명제 7](/ko/note/set_theory/sum_of_sets#pp7)에 의하여 우리는 cardinal들의 합을 mutually disjoint family의 합집합으로 취급할 수 있다. 이제, 첫 번째 식들은 각각 [§합집합과 교집합, 명제 4](/ko/note/set_theory/union_and_intersection#pp4), [§집합의 곱, 명제 5](/ko/note/set_theory/product_of_sets#pp5)의 결과들이며, 두 번째 식들은 [§합집합과 교집합, 명제 5](/ko/note/set_theory/union_and_intersection#pp5)와 [§집합의 곱, 명제 8](/ko/note/set_theory/product_of_sets#pp8)의 결과들이다.
...
공집합과 합집합을 하는 것과, 원소 하나짜리 집합과 곱을 하는 것은 cadrinal에 영향을 미치지 않는다. ($x\mapsto (x,i)$가 $X$에서 $X\times\{i\}$로의 bijection을 정의한다)
...
[§순서쌍, 명제7](/ko/note/set_theory/ordered_pair#pp10)의 임의의 product로의 확장. 증명도 동일하게 하면 된다.
...
$F^E$와 $\operatorname{Fun}(E,F)$ 간의 bijection이 존재하므로 자명하다.
...
$\mathbf{2}=\\{\alpha, \beta\\}$가 cardinal이라 하자. 임의의 $Y\in\mathcal{P}(X)$에 대하여, 만약 $x\in Y$라면 $x\mapsto\alpha$이고, 그렇지 않을 때에는 $x\mapsto\beta$인 함수 $f_Y:X\rightarrow \mathbf{2}$가 존재한다. 반대로 임의의 함수 $f:X\rightarrow \mathbf{2}$에 대하여, $f^{-1}(\alpha)$가 $\mathcal{P}(X)$의 원소를 하나 지정한다.
